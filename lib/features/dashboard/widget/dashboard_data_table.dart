...
class _DashboardDataTableState extends State<DashboardDataTable> {
          ...
          Row(
            children: [
              Expanded(
                child: Text(
                  widget.title,
                  style: const TextStyle(color: Color(0xFF17243A), fontSize: 16, fontWeight: FontWeight.w800),
                ),
              ),
              Container(
          ...
          SingleChildScrollView(
            ...
            child: DataTable(
              ...
              columns: widget.columns
                  .map((c) => DataColumn(label: Text(c, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 12))))
                  .toList(),
              rows: _visibleRows.map((r) {
                return DataRow(
                  cells: r.map((cell) => DataCell(Text(cell, style: const TextStyle(fontSize: 12)))).toList(),
                );
              }).toList(),
            ),
          ),
  ...
  Widget _buildPager() {
    ...
    final int end = (start - 1 + widget.rowsPerPage).clamp(0, widget.rows.length);
...
class _PagerButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      color: enabled ? const Color(0xFFEAF3FF) : const Color(0xFFF1F3F6),
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Icon(
            icon,
            size: 19,
            color: enabled ? const Color(0xFF1565C0) : const Color(0xFFB6BEC9),
          ),
        ),
      ),
    );
